...
error_t rtl8211Init(NetInterface *interface)
{
   ...
   if(interface->phyAddr >= 32)
   {
      ...
      interface->phyAddr = RTL8211_PHY_ADDR;
   }
   ...
   rtl8211WritePhyReg(interface, RTL8211_BMCR, RTL8211_BMCR_RESET);

   //Wait for the reset to complete
   while(rtl8211ReadPhyReg(interface, RTL8211_BMCR) & RTL8211_BMCR_RESET)
   {
   }
   ...
   rtl8211WritePhyReg(interface, RTL8211_INER, RTL8211_INER_AN_COMPLETE |
      RTL8211_INER_LINK_STATUS);
...
void rtl8211Tick(NetInterface *interface)
{
   ...
   if(interface->extIntDriver == NULL)
   {
      ...
      value = rtl8211ReadPhyReg(interface, RTL8211_BMSR);
      //Retrieve current link state
      linkState = (value & RTL8211_BMSR_LINK_STATUS) ? TRUE : FALSE;
...
void rtl8211EventHandler(NetInterface *interface)
{
   ...
   status = rtl8211ReadPhyReg(interface, RTL8211_INSR);

   //Link status change?
   if(status & (RTL8211_INSR_AN_COMPLETE | RTL8211_INSR_LINK_STATUS))
   {
      //Any link failure condition is latched in the BMSR register. Reading
      //the register twice will always return the actual link status
      status = rtl8211ReadPhyReg(interface, RTL8211_BMSR);
      status = rtl8211ReadPhyReg(interface, RTL8211_BMSR);

      //Link is up?
      if(status & RTL8211_BMSR_LINK_STATUS)
      {
         //Read PHY status register
         status = rtl8211ReadPhyReg(interface, RTL8211_PHYSR);

         //Check current speed
         switch(status & RTL8211_PHYSR_SPEED)
         {
         //10BASE-T
         case RTL8211_PHYSR_SPEED_10MBPS:
            interface->linkSpeed = NIC_LINK_SPEED_10MBPS;
            break;
         //100BASE-TX
         case RTL8211_PHYSR_SPEED_100MBPS:
            interface->linkSpeed = NIC_LINK_SPEED_100MBPS;
            break;
         //1000BASE-T
         case RTL8211_PHYSR_SPEED_1000MBPS:
         ...
         if(status & RTL8211_PHYSR_DUPLEX)
